...
class ModManagerGUI:
    ...
    def setup_gui(self):
        ...
        ttk.Label(top_frame, text="XML File:").pack(side=tk.LEFT)
        self.file_label = ttk.Label(top_frame, text="No file selected", foreground="gray")
        self.file_label.pack(side=tk.LEFT, padx=(10, 0))
        
        ttk.Button(top_frame, text="Browse XML", command=self.browse_xml_file).pack(side=tk.RIGHT)
        ttk.Button(top_frame, text="Load XML", command=self.load_xml_file).pack(side=tk.RIGHT, padx=(0, 5))
        
        # Create single treeview for all items
    ...
    def reset_changes(self):
        ...
        if messagebox.askyesno("Confirm Reset", "Reset all changes? This will reload from the XML file."):
            self.load_xml_file()
